...
| `/merchants` | GET | `q`（商户名关键词，空=全部商户）, `grid?` | **聚合 API**：按商户分组返回每家商户的全部商品名列表（含 unit_id 可直接比价，无分页）|
| `/api/discover` | GET | `q`（商户名模糊）, `grid?`, `merchant_limit`(默认20), `product_limit`(默认100), `product_offset` | **商户发现 API**：模糊商户搜索（商户上限20，网格过滤+地点排名）+ 命中商户下商品扁平化**分页**（100/页），返回 merchant_total/product_total 等计数 |
| `/export` | POST | `{items:[{unit_id, qty?, include_delivery?, first_order?}]}` | 把一组比价结果导出为**带时间戳的 PDF**（reportlab，内置中文字体）；`Content-Disposition` 附件下载，文件名 `comparison_YYYYMMDD_HHMMSS.pdf`；空/超100→400，未知单元→404 |
| `/health` | GET | - | 服务状态、单元/条目数 |
| `/` | GET | - | 正式客户比价界面（app.html）|
| `/discover` | GET | - | 商户发现界面（GPS+模糊商户搜索+商品分页，discover.html）|
| `/test` | GET | - | 测试+报表控制台（test.html）|
