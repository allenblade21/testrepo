...
# 起送价为「商户(门店) × 平台」级属性——各平台的起送价由商家自设、逐店不同，
# 不存在平台统一起送价（BUG-002 修复：此前误建为平台级，导致瑞幸美团
# 不足 20 元被错误拦截；瑞幸等低门槛门店未配置即为 0，可正常下单）。
# 键: (商户, 平台) → 起送价（分）；未配置默认 0
STORE_MIN_ORDER = {
    ("喜茶(国贸店)", "美团"): 2600,
    ("喜茶(国贸店)", "阿里闪购"): 1500,
}

# 每日首单券（按平台）：(描述, 面额分, 使用门槛分)
# 用户当日尚未在该平台下过单时可用，由比价请求按用户勾选传入
FIRST_ORDER_COUPON = {
    "阿里闪购": ("每日首单减4", 400, 1000),
    "京东": ("每日首单减3", 300, 0),
    "美团": ("每日首单减5", 500, 1500),
}
