...
>
> **新垂直立项**：[docs/团餐团购比价需求.md](docs/团餐团购比价需求.md)——餐厅到店团购套餐 / 多人团餐的跨平台比价（平台集与到手价口径均不同于饮品，独立评审）；G-R1 设计见 [docs/团购比价设计文档.md](docs/团购比价设计文档.md)（决策 ADR-010~013），数据源接入见 [docs/CPS接入条件与步骤.md](docs/CPS接入条件与步骤.md)。
...
- **导出 PDF**：一键把会话内所有比价结果导出为带时间戳的 PDF（`POST /export`，reportlab 中文渲染）
