...
## [v0.9.0] G-R2 匹配置信度分级 + G-R3 团购比价界面 /tuangou
- **G-R2 置信度分级**（多信号，`matching.py`）：跨平台单元按【标题相似度（归一化字符二元组 Jaccard ≥0.6）+ 价位带（min/max ≥0.75）】双信号判定——**精确**（都过）/**疑似**（任一不过，`match_note` 给可解释差异原因）/**单平台**；阈值有测试锁。三源海底捞单元由「疑似」合法升级「精确」。
- **G-R3 界面 `/tuangou`**（web/tuangou.html，ADR-013 分路径兑现）：人数筛选 + 搜索（防抖+过期响应丢弃）→ 可比单元卡（置信度徽章/平台列表/团购价起）→ 展开即比价（平台并排、人均高亮、最划算徽章、明细行、菜品/降级标注、使用规则、deeplink、人均节省）；数据源探针行 + Mock 标注；三入口导航互通。
- **E2E `tests/e2e/tuangou.spec.mjs`**（9 断言全过，实测人均 ¥47.25 与后端 4725 分一致）并入 CI（第 5 个 E2E）。
- pytest 196→**203 全绿 + 1 skip**（新增置信度专项 8 项：相似度/双信号/价位带边界 0.75/最弱对决定/单平台不受影响）。
